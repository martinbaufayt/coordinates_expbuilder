...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  const changeFormat = translate('changeFormat')
  // global variable
  ...
  const projectMapPoint = (point, selectedSystem, threeDPoint?) => {
    const { wkid, decimal, datumWkid, datumWkid2, transformForward, transformForward2, displayUnit, crs } = selectedSystem
    if (!wkid) return
    const curWkidNum = parseInt(wkid)
    const curDecimal = parseInt(decimal)
    console.log(curDecimal);

    // Format override: project to the geographic CRS of the configured system, then apply the selected format
    if (selectedDisplayFormat) {
      const spheroidStr = getWktKeyStr(crs, 'GEOGCS')
      const outCrs = getCrsBySpheroidStr(spheroidStr)
      const geoWkid = outCrs?.wkid ?? 4326
      let geoPoint: any = projection.project(point, new SpatialReference({ wkid: geoWkid }))
      if (!geoPoint) {
        geoPoint = new Point({ x: point.longitude ?? point.x, y: point.latitude ?? point.y, spatialReference: new SpatialReference({ wkid: geoWkid }) })
      }
      setGeoInfo(computing)
      switch (selectedDisplayFormat) {
        case 'UTM': displayUtm(geoPoint); break
        case 'LON_LAT': displayLonLat(geoPoint); break
        case 'MGRS': displayUsngOrMgrs('MGRS', geoPoint); break
        case 'USNG': displayUsngOrMgrs('USNG', geoPoint); break
        default: displayDegOrDms(selectedDisplayFormat, { x: geoPoint.longitude ?? geoPoint.x, y: geoPoint.latitude ?? geoPoint.y }, false, 'DEGREE')
      }
      const view = currentJimuMapView?.view
      if (view?.type === '3d') {
        _setEyeInfo()
  ...
  const handleFormatChange = (format: string) => {
    setSelectedDisplayFormat(format)
    setGeoInfo('')
    setElevInfo('')
    setElevNum(null)
    setElevUnit('')
  ...
  const toFormat = (num) => {
    ...
    //console.log(selectedSystem.decimal);

    if(isNaN(selectedSystem.decimal))
    {
      if (isNaN(num)) return ''
  ...
  const displayOnClient = (mapPoint, threeDPoint?) => {
    ...
    if (selectedDisplayFormat) {
      ...
      const outCrs = getCrsBySpheroidStr(spheroidStr)
      const geoWkid = outCrs?.wkid ?? 4326
      let geoPoint: any = projection.project(copyMapPoint, new SpatialReference({ wkid: geoWkid }))
      if (!geoPoint) {
        const np = normalizedPoint
        geoPoint = new Point({ x: np.longitude ?? np.x, y: np.latitude ?? np.y, spatialReference: new SpatialReference({ wkid: geoWkid }) })
      }
      switch (selectedDisplayFormat) {
        case 'UTM': displayUtm(geoPoint); break
        case 'LON_LAT': displayLonLat(geoPoint); break
          ...
          <Dropdown size='sm' activeIcon>
            ...
            <DropdownMenu>
              {formatOptions.map(option => (
                <DropdownItem key={option.value} active={selectedDisplayFormat === option.value} onClick={() => handleFormatChange(option.value)}>
                  {option.label}
                </DropdownItem>
              ))}
            </DropdownMenu>
