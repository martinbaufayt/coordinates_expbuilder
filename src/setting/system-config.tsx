/** @jsx jsx */
import { React, jsx, type IMThemeVariables, type ImmutableArray, hooks, WidgetJson, DataSourceJson } from 'jimu-core'
...
export const SystemConfig = (props: SystemConfigProps & CoordinateConfig): React.ReactElement => {
  const {
    id, name, wkid, decimal, crs, displayUnit, elevationUnit, datumWkid, datumName, transformForward, datumWkid2, datumName2, transformForward2,
  ...
  const [curDecimal, setCurDecimal] = useState(decimal)
  ...
  const specificDisplayOptions = translate('specificDisplayOptions')
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiLoaded, wkid, name, crs, decimal,  datumWkid, mapInfo, mapInfo2, wkidUtils])
  ...
  const decimalWkidChange = (value): void => {
    setCurDecimal(value)
     multiOptionsChange({
        decimal: value
      })
      ...
      <div className='w-100 h-100 system-config-panel'>
        {allDataLoaded
          ? <Fragment>
            ...
            <SettingSection
                        title={specificDisplayOptions}
                        role='group'
                        aria-label={specificDisplayOptions}
                      >
